...
async fn main() {
    ...
    let response = client
        ...
        .line_items(
            vec![
                LineItemCreate { tax_code : Some("your tax code".to_owned()), unit_amount
                : 1.0, item_id : Some("your item id".to_owned()), revenue_schedule_type :
                Some("your revenue schedule type".to_owned()), avalara_service_type :
                Some(1), quantity : Some(1), tax_inclusive : Some(true),
                avalara_transaction_type : Some(1), start_date : Some("your start date"
                .to_owned()), end_date : Some("your end date".to_owned()), tax_exempt :
                Some(true), origin : Some("your origin".to_owned()), currency :
                "your currency".to_owned(), description : Some("your description"
                .to_owned()), type_ : "your type".to_owned(), credit_reason_code :
                Some("your credit reason code".to_owned()), accounting_code :
                Some("your accounting code".to_owned()), item_code :
                Some("your item code".to_owned()), product_code :
                Some("your product code".to_owned()) }
            ],
        )
        .subscriptions(
            vec![
                SubscriptionPurchase { tax_inclusive : Some(true), renewal_billing_cycles
                : Some(1), quantity : Some(1), total_billing_cycles : Some(1), plan_code
                : "your plan code".to_owned(), unit_amount : Some(1.0), shipping :
                ...
                Some(1.0) }), trial_ends_at : Some("your trial ends at".to_owned()),
                next_bill_date : Some("your next bill date".to_owned()), auto_renew :
                Some(true), revenue_schedule_type : Some("your revenue schedule type"
                .to_owned()), custom_fields : Some(CustomFields(vec![CustomField { name :
                "your name".to_owned(), value : "your value".to_owned() }])), starts_at :
                Some("your starts at".to_owned()), ramp_intervals :
                Some(vec![SubscriptionRampInterval { starting_billing_cycle : Some(1),
                unit_amount : Some(1) }]), add_ons : Some(vec![SubscriptionAddOnCreate {
