...
async fn main() {
    ...
    let response = client
        ...
        .line_items(
            vec![
                LineItemCreate { currency : "your currency".to_owned(), description :
                Some("your description".to_owned()), credit_reason_code :
                Some("your credit reason code".to_owned()), avalara_transaction_type :
                Some(1), item_id : Some("your item id".to_owned()), avalara_service_type
                : Some(1), tax_inclusive : Some(true), origin : Some("your origin"
                .to_owned()), tax_code : Some("your tax code".to_owned()), end_date :
                Some("your end date".to_owned()), accounting_code :
                Some("your accounting code".to_owned()), revenue_schedule_type :
                Some("your revenue schedule type".to_owned()), start_date :
                Some("your start date".to_owned()), item_code : Some("your item code"
                .to_owned()), type_ : "your type".to_owned(), unit_amount : 1.0, quantity
                : Some(1), product_code : Some("your product code".to_owned()),
                tax_exempt : Some(true) }
            ],
        )
        .subscriptions(
            vec![
                SubscriptionPurchase { tax_inclusive : Some(true), add_ons :
                Some(vec![SubscriptionAddOnCreate { quantity : Some(1), usage_percentage
                : Some(1.0), unit_amount_decimal : Some("your unit amount decimal"
                .to_owned()), tiers : Some(vec![SubscriptionAddOnTier { usage_percentage
                : Some("your usage percentage".to_owned()), unit_amount_decimal :
                Some("your unit amount decimal".to_owned()), ending_quantity : Some(1),
                unit_amount : Some(1.0) }]), percentage_tiers :
                Some(vec![SubscriptionAddOnPercentageTier { ending_amount : Some(1.0),
                usage_percentage : Some("your usage percentage".to_owned()) }]), code :
                "your code".to_owned(), unit_amount : Some(1.0), add_on_source :
                Some("your add on source".to_owned()), revenue_schedule_type :
                Some("your revenue schedule type".to_owned()) }]), starts_at :
                Some("your starts at".to_owned()), revenue_schedule_type :
                Some("your revenue schedule type".to_owned()), quantity : Some(1),
                plan_id : Some("your plan id".to_owned()), total_billing_cycles :
                Some(1), custom_fields : Some(CustomFields(vec![CustomField { value :
                "your value".to_owned(), name : "your name".to_owned() }])),
                trial_ends_at : Some("your trial ends at".to_owned()), plan_code :
                "your plan code".to_owned(), auto_renew : Some(true), unit_amount :
                Some(1.0), renewal_billing_cycles : Some(1), ramp_intervals :
                Some(vec![SubscriptionRampInterval { starting_billing_cycle : Some(1),
                unit_amount : Some(1) }]), next_bill_date : Some("your next bill date"
                .to_owned()), shipping : Some(SubscriptionShippingPurchase { method_id :
                Some("your method id".to_owned()), amount : Some(1.0), method_code :
                Some("your method code".to_owned()) }) }
            ],
        )
