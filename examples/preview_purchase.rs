...
async fn main() {
    ...
    let response = client
        ...
        .line_items(
            vec![
                LineItemCreate { tax_inclusive : Some(true), credit_reason_code :
                Some("your credit reason code".to_owned()), item_code :
                Some("your item code".to_owned()), accounting_code :
                Some("your accounting code".to_owned()), currency : "your currency"
                .to_owned(), origin : Some("your origin".to_owned()),
                avalara_transaction_type : Some(1), unit_amount : 1.0, tax_code :
                Some("your tax code".to_owned()), revenue_schedule_type :
                Some("your revenue schedule type".to_owned()), avalara_service_type :
                Some(1), item_id : Some("your item id".to_owned()), type_ : "your type"
                .to_owned(), product_code : Some("your product code".to_owned()),
                start_date : Some("your start date".to_owned()), quantity : Some(1),
                description : Some("your description".to_owned()), tax_exempt :
                Some(true), end_date : Some("your end date".to_owned()) }
            ],
        )
        .subscriptions(
            vec![
                SubscriptionPurchase { plan_code : "your plan code".to_owned(),
                next_bill_date : Some("your next bill date".to_owned()),
                revenue_schedule_type : Some("your revenue schedule type".to_owned()),
                plan_id : Some("your plan id".to_owned()), unit_amount : Some(1.0),
                add_ons : Some(vec![SubscriptionAddOnCreate { unit_amount_decimal :
                Some("your unit amount decimal".to_owned()), percentage_tiers :
                Some(vec![SubscriptionAddOnPercentageTier { ending_amount : Some(1.0),
                usage_percentage : Some("your usage percentage".to_owned()) }]), quantity
                : Some(1), unit_amount : Some(1.0), usage_percentage : Some(1.0), code :
                ...
                ending_quantity : Some(1), unit_amount : Some(1.0) }]),
                revenue_schedule_type : Some("your revenue schedule type".to_owned())
                }]), trial_ends_at : Some("your trial ends at".to_owned()), starts_at :
                Some("your starts at".to_owned()), shipping :
                ...
                Some(1.0) }), total_billing_cycles : Some(1), quantity : Some(1),
                tax_inclusive : Some(true), auto_renew : Some(true), custom_fields :
                Some(CustomFields(vec![CustomField { name : "your name".to_owned(), value
                : "your value".to_owned() }])), ramp_intervals :
                Some(vec![SubscriptionRampInterval { unit_amount : Some(1),
                starting_billing_cycle : Some(1) }]), renewal_billing_cycles : Some(1) }
            ],
        )
