...
async fn main() {
    ...
    let response = client
        ...
        .line_items(
            vec![
                LineItemCreate { unit_amount : 1.0, revenue_schedule_type :
                Some("your revenue schedule type".to_owned()), end_date :
                Some("your end date".to_owned()), tax_code : Some("your tax code"
                .to_owned()), credit_reason_code : Some("your credit reason code"
                .to_owned()), quantity : Some(1), item_id : Some("your item id"
                .to_owned()), avalara_service_type : Some(1), avalara_transaction_type :
                Some(1), product_code : Some("your product code".to_owned()),
                tax_inclusive : Some(true), accounting_code : Some("your accounting code"
                .to_owned()), origin : Some("your origin".to_owned()), description :
                Some("your description".to_owned()), type_ : "your type".to_owned(),
                currency : "your currency".to_owned(), start_date :
                Some("your start date".to_owned()), tax_exempt : Some(true), item_code :
                Some("your item code".to_owned()) }
            ],
        )
        .subscriptions(
            vec![
                SubscriptionPurchase { custom_fields : Some(CustomFields(vec![CustomField
                { value : "your value".to_owned(), name : "your name".to_owned() }])),
                starts_at : Some("your starts at".to_owned()), next_bill_date :
                Some("your next bill date".to_owned()), auto_renew : Some(true), plan_id
                : Some("your plan id".to_owned()), plan_code : "your plan code"
                .to_owned(), revenue_schedule_type : Some("your revenue schedule type"
                .to_owned()), unit_amount : Some(1.0), add_ons :
                Some(vec![SubscriptionAddOnCreate { quantity : Some(1), code :
                "your code".to_owned(), percentage_tiers :
                Some(vec![SubscriptionAddOnPercentageTier { usage_percentage :
                Some("your usage percentage".to_owned()), ending_amount : Some(1.0) }]),
                tiers : Some(vec![SubscriptionAddOnTier { usage_percentage :
                Some("your usage percentage".to_owned()), unit_amount_decimal :
                Some("your unit amount decimal".to_owned()), unit_amount : Some(1.0),
                ending_quantity : Some(1) }]), add_on_source : Some("your add on source"
                .to_owned()), unit_amount : Some(1.0), usage_percentage : Some(1.0),
                ...
                starting_billing_cycle : Some(1), unit_amount : Some(1) }]),
                total_billing_cycles : Some(1), renewal_billing_cycles : Some(1),
                quantity : Some(1), trial_ends_at : Some("your trial ends at".to_owned())
                }
            ],
        )
