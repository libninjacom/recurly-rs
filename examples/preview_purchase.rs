...
async fn main() {
    ...
    let response = client
        ...
        .line_items(
            vec![
                LineItemCreate { type_ : "your type".to_owned(), tax_code :
                Some("your tax code".to_owned()), origin : Some("your origin"
                .to_owned()), avalara_service_type : Some(1), unit_amount : 1.0,
                ...
                Some("your description".to_owned()), tax_exempt : Some(true), currency :
                "your currency".to_owned(), quantity : Some(1), tax_inclusive :
                Some(true), product_code : Some("your product code".to_owned()),
                accounting_code : Some("your accounting code".to_owned()), end_date :
                Some("your end date".to_owned()), revenue_schedule_type :
                Some("your revenue schedule type".to_owned()), credit_reason_code :
                Some("your credit reason code".to_owned()) }
            ],
        )
        .subscriptions(
            vec![
                SubscriptionPurchase { renewal_billing_cycles : Some(1), add_ons :
                ...
                usage_percentage : Some(1.0) }]), plan_id : Some("your plan id"
                .to_owned()), shipping : Some(SubscriptionShippingPurchase { amount :
                Some(1.0), method_id : Some("your method id".to_owned()), method_code :
                Some("your method code".to_owned()) }), ramp_intervals :
                Some(vec![SubscriptionRampInterval { unit_amount : Some(1),
                starting_billing_cycle : Some(1) }]), starts_at : Some("your starts at"
                .to_owned()), unit_amount : Some(1.0), quantity : Some(1), next_bill_date
                : Some("your next bill date".to_owned()), auto_renew : Some(true),
                tax_inclusive : Some(true), total_billing_cycles : Some(1), custom_fields
                : Some(CustomFields(vec![CustomField { value : "your value".to_owned(),
                name : "your name".to_owned() }])), trial_ends_at :
                Some("your trial ends at".to_owned()), revenue_schedule_type :
                Some("your revenue schedule type".to_owned()), plan_code :
                "your plan code".to_owned() }
            ],
        )
