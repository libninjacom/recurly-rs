...
async fn main() {
    ...
    let response = client
        ...
        .line_items(
            vec![
                LineItemCreate { accounting_code : Some("your accounting code"
                .to_owned()), avalara_service_type : Some(1), tax_exempt : Some(true),
                product_code : Some("your product code".to_owned()), item_code :
                Some("your item code".to_owned()), end_date : Some("your end date"
                .to_owned()), tax_inclusive : Some(true), description :
                ...
                Some("your start date".to_owned()), unit_amount : 1.0, currency :
                "your currency".to_owned(), credit_reason_code :
                Some("your credit reason code".to_owned()), item_id : Some("your item id"
                .to_owned()), origin : Some("your origin".to_owned()), type_ :
                "your type".to_owned(), avalara_transaction_type : Some(1) }
            ],
        )
        .subscriptions(
            vec![
                SubscriptionPurchase { ramp_intervals :
                Some(vec![SubscriptionRampInterval { starting_billing_cycle : Some(1),
                unit_amount : Some(1) }]), total_billing_cycles : Some(1),
                revenue_schedule_type : Some("your revenue schedule type".to_owned()),
                tax_inclusive : Some(true), add_ons : Some(vec![SubscriptionAddOnCreate {
                ...
                .to_owned(), unit_amount : Some(1.0), quantity : Some(1),
                percentage_tiers : Some(vec![SubscriptionAddOnPercentageTier {
                usage_percentage : Some("your usage percentage".to_owned()),
                ending_amount : Some(1.0) }]), add_on_source : Some("your add on source"
                .to_owned()), usage_percentage : Some(1.0) }]), trial_ends_at :
                Some("your trial ends at".to_owned()), custom_fields :
                Some(CustomFields(vec![CustomField { name : "your name".to_owned(), value
                : "your value".to_owned() }])), starts_at : Some("your starts at"
                .to_owned()), quantity : Some(1), next_bill_date :
                Some("your next bill date".to_owned()), renewal_billing_cycles : Some(1),
                shipping : Some(SubscriptionShippingPurchase { method_code :
                Some("your method code".to_owned()), amount : Some(1.0), method_id :
                Some("your method id".to_owned()) }), auto_renew : Some(true), plan_code
                : "your plan code".to_owned(), plan_id : Some("your plan id".to_owned()),
