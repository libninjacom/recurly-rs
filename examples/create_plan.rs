...
async fn main() {
    ...
    let name = "your name";
    let currencies = vec![
        PlanPricing { currency : Some("your currency".to_owned()), unit_amount :
        Some(1.0), setup_fee : Some(1.0), tax_inclusive : Some(true) }
    ];
    let response = client
        ...
        .add_ons(
            vec![
                AddOnCreate { item_code : Some("your item code".to_owned()), optional :
                Some(true), measured_unit_name : Some("your measured unit name"
                .to_owned()), tax_code : Some("your tax code".to_owned()),
                usage_percentage : Some(1.0), usage_calculation_type :
                Some("your usage calculation type".to_owned()), usage_type :
                Some("your usage type".to_owned()), tiers : Some(vec![Tier {
                ...
                currency : "your currency".to_owned() }]) }]), percentage_tiers :
                Some(vec![PercentageTiersByCurrency { currency : Some("your currency"
                .to_owned()), tiers : Some(vec![PercentageTier { usage_percentage :
                Some("your usage percentage".to_owned()), ending_amount : Some(1.0) }])
                }]), code : "your code".to_owned(), display_quantity : Some(true), name :
                "your name".to_owned(), avalara_transaction_type : Some(1), plan_id :
                Some("your plan id".to_owned()), default_quantity : Some(1), currencies :
                Some(vec![AddOnPricing { currency : "your currency".to_owned(),
                unit_amount : Some(1.0), tax_inclusive : Some(true), unit_amount_decimal
                : Some("your unit amount decimal".to_owned()) }]), item_id :
                Some("your item id".to_owned()), add_on_type : Some("your add on type"
                .to_owned()), revenue_schedule_type : Some("your revenue schedule type"
                .to_owned()), avalara_service_type : Some(1), tier_type :
                Some("your tier type".to_owned()), usage_timeframe :
                Some("your usage timeframe".to_owned()), accounting_code :
