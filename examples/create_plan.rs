...
async fn main() {
    ...
    let name = "your name";
    let currencies = vec![
        PlanPricing { currency : Some("your currency".to_owned()), setup_fee : Some(1.0),
    ...
    let response = client
        ...
        .add_ons(
            vec![
                AddOnCreate { revenue_schedule_type : Some("your revenue schedule type"
                .to_owned()), avalara_transaction_type : Some(1), avalara_service_type :
                Some(1), usage_timeframe : Some("your usage timeframe".to_owned()), tiers
                : Some(vec![Tier { currencies : Some(vec![TierPricing {
                unit_amount_decimal : Some("your unit amount decimal".to_owned()),
                unit_amount : Some(1.0), currency : "your currency".to_owned() }]),
                ending_quantity : Some(1), usage_percentage :
                Some("your usage percentage".to_owned()) }]), code : "your code"
                .to_owned(), percentage_tiers : Some(vec![PercentageTiersByCurrency {
                currency : Some("your currency".to_owned()), tiers :
                Some(vec![PercentageTier { ending_amount : Some(1.0), usage_percentage :
                Some("your usage percentage".to_owned()) }]) }]), name : "your name"
                .to_owned(), display_quantity : Some(true), optional : Some(true),
                usage_percentage : Some(1.0), item_id : Some("your item id".to_owned()),
                measured_unit_id : Some("your measured unit id".to_owned()),
                usage_calculation_type : Some("your usage calculation type".to_owned()),
                add_on_type : Some("your add on type".to_owned()), tier_type :
                Some("your tier type".to_owned()), usage_type : Some("your usage type"
                .to_owned()), plan_id : Some("your plan id".to_owned()),
                measured_unit_name : Some("your measured unit name".to_owned()),
                currencies : Some(vec![AddOnPricing { tax_inclusive : Some(true),
                ...
                .to_owned()), item_code : Some("your item code".to_owned()), tax_code :
                Some("your tax code".to_owned()), default_quantity : Some(1) }
            ],
        )
