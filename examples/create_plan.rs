...
async fn main() {
    ...
    let response = client
        ...
        .add_ons(
            vec![
                AddOnCreate { tax_code : Some("your tax code".to_owned()), plan_id :
                Some("your plan id".to_owned()), tier_type : Some("your tier type"
                ...
                .to_owned()), currencies : Some(vec![TierPricing { currency :
                "your currency".to_owned(), unit_amount : Some(1.0), unit_amount_decimal
                : Some("your unit amount decimal".to_owned()) }]), ending_quantity :
                Some(1) }]), add_on_type : Some("your add on type".to_owned()),
                usage_type : Some("your usage type".to_owned()), revenue_schedule_type :
                Some("your revenue schedule type".to_owned()), avalara_service_type :
                Some(1), percentage_tiers : Some(vec![PercentageTiersByCurrency { tiers :
                Some(vec![PercentageTier { ending_amount : Some(1.0), usage_percentage :
                Some("your usage percentage".to_owned()) }]), currency :
                Some("your currency".to_owned()) }]), default_quantity : Some(1), item_id
                : Some("your item id".to_owned()), item_code : Some("your item code"
                .to_owned()), currencies : Some(vec![AddOnPricing { unit_amount_decimal :
                Some("your unit amount decimal".to_owned()), currency : "your currency"
                .to_owned(), tax_inclusive : Some(true), unit_amount : Some(1.0) }]),
                name : "your name".to_owned(), optional : Some(true), measured_unit_name
                : Some("your measured unit name".to_owned()), accounting_code :
                Some("your accounting code".to_owned()), measured_unit_id :
                Some("your measured unit id".to_owned()), display_quantity : Some(true),
