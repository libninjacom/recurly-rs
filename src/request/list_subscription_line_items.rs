...
pub struct ListSubscriptionLineItemsRequest<'a> {
...
impl<'a> ListSubscriptionLineItemsRequest<'a> {
    pub async fn send(self) -> anyhow::Result<LineItemList> {
        let mut r = self
            .client
            .client
            .get(
                &format!(
                    "/subscriptions/{subscription_id}/line_items", subscription_id = self
                    .subscription_id
                ),
            );
        ...
        if let Some(ref unwrapped) = self.original {
            r = r.push_query("original", &unwrapped.to_string());
        }
    ...
    pub fn original(mut self, original: serde_json::Value) -> Self {
