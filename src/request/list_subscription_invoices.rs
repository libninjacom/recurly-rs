...
pub struct ListSubscriptionInvoicesRequest<'a> {
...
impl<'a> ListSubscriptionInvoicesRequest<'a> {
    pub async fn send(self) -> anyhow::Result<InvoiceList> {
        let mut r = self
            .http_client
            .client
            .get(
                &format!(
                    "/subscriptions/{subscription_id}/invoices", subscription_id = self
                    .subscription_id
                ),
            );
