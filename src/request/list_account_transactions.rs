...
pub struct ListAccountTransactionsRequest<'a> {
...
impl<'a> ListAccountTransactionsRequest<'a> {
    pub async fn send(self) -> anyhow::Result<TransactionList> {
        let mut r = self
            .client
            .client
            .get(
                &format!(
                    "/accounts/{account_id}/transactions", account_id = self.account_id
                ),
            );
        ...
        if let Some(ref unwrapped) = self.success {
            r = r.push_query("success", &unwrapped.to_string());
        }
    ...
    pub fn success(mut self, success: serde_json::Value) -> Self {
