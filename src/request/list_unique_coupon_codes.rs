...
pub struct ListUniqueCouponCodesRequest<'a> {
...
impl<'a> ListUniqueCouponCodesRequest<'a> {
    pub async fn send(self) -> anyhow::Result<UniqueCouponCodeList> {
        let mut r = self
            .http_client
            .client
            .get(
                &format!(
                    "/coupons/{coupon_id}/unique_coupon_codes", coupon_id = self
                    .coupon_id
                ),
            );
